...
//! can change onChange(setFromData) and use on the parent form element , before onSubmit or after
  return (
    <form className="new-task-form" onSubmit={handleFormSubmit}>
      <label>
        Details
        <input type="text" name="text" value={inputText} onChange={e => setInputText(e.target.value)} required/>
      </label>
      <label>
        Category
        <select name="category" text='Select a Category' value={categorySelect} onChange={e => setCategorySelect(e.target.value)} required>
